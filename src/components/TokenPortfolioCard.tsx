...
export function TokenPortfolioCard({
    ...
    apiResponse?.usdPrice24hrUsdChange !== null
      ? Math.abs(apiResponse?.usdPrice24hrUsdChange) < 0.01
        ? isPositiveChange
          ...
          : "-<$0.01"
        : isPositiveChange
          ? "+$" + Math.abs(apiResponse?.usdPrice24hrUsdChange).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
          : "-$" + Math.abs(apiResponse?.usdPrice24hrUsdChange).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : "Loading..."
  ...
  return (
    <Card className="w-full border-0 bg-[#171923] hover:bg-[#1A202C] transition-all duration-200 overflow-hidden mb-3 text-lg">
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h3 className="font-bold text-lg text-white tracking-wide">{tokenData.name}</h3>
            <p className="text-base text-gray-400 font-medium">{formattedAmount} {tokenData.symbol}</p>
          </div>
        </div>
